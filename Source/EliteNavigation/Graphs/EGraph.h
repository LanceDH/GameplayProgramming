...
namespace Elite
{
	...
	public:
		...
		//Search for the closest node in the graph
		T* GetClosestNodeAny(const Vector2& p, Heuristic heuristicFunction) {
			//Local variables
			T* pClosestNode = nullptr;
			unsigned int pAmountOfActiveNodes = 0;
			auto closestSqrtDistance = (std::numeric_limits<float>::max)();
		...
		T* GetNeighbourInDirection(T* pNode, const Vector2& dir, int rowSize) {
			vector<T*> vNodes = m_Nodes.GetAllActiveUnits();

			Node* pBaseNode = static_cast<Node*>(pNode);
			Node* pOtherBaseNode;

			auto it = find(vNodes.begin(), vNodes.end(), pNode);
			int FUCKOFF = 0;
			int j = it - vNodes.begin();
			float bestDot = -1;
			//float bestCon = vConnections[0];
			T* bestNode = nullptr;
			...
			if (FUCKOFF > 0) {
				T* left = vNodes[j - 1];
				pOtherBaseNode = static_cast<Node*>(left);
				float dot = Dot(dir, pOtherBaseNode->GetPosition() - pBaseNode->GetPosition());
				if (dot > bestDot) {
					bestDot = dot;
					bestNode = left;
				}
			...
			FUCKOFF = j + 1;
			if (FUCKOFF < vNodes.size()) {
				T* right = vNodes[j + 1];
				pOtherBaseNode = static_cast<Node*>(right);
				float dot = Dot(dir, pOtherBaseNode->GetPosition() - pBaseNode->GetPosition());
				if (dot > bestDot) {
					bestDot = dot;
					bestNode = right;
			...
			FUCKOFF = j - rowSize;
			if (FUCKOFF > 0) {
				T* up = vNodes[j - rowSize];
				pOtherBaseNode = static_cast<Node*>(up);
				float dot = Dot(dir, pOtherBaseNode->GetPosition() - pBaseNode->GetPosition());
				if (dot > bestDot) {
					bestDot = dot;
					bestNode = up;
			...
			FUCKOFF = j + rowSize;
			if (FUCKOFF < vNodes.size()) {
				T* down = vNodes[j + rowSize];
				pOtherBaseNode = static_cast<Node*>(down);
				float dot = Dot(dir, pOtherBaseNode->GetPosition() - pBaseNode->GetPosition());
				if (dot > bestDot) {
					bestDot = dot;
					bestNode = down;
